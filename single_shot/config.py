...
class SystemParams:
    Pattern = "([^(]*)"
    seed = 0
    use_multiprocessing = False  # split video into chunks and process them in parallel
...
@dataclass
class FaceParams:
    # keys_to_use_for_estimation_pairs = [('x2', 'y2')]
    keys_to_use_for_estimation_pairs = [('center_x', 'center_y')]
    # keys_to_use_for_estimation = ['x2', 'y2', 'frame_no']
    keys_to_use_for_estimation = ['center_x', 'center_y', 'frame_no']
    # keys_to_use_for_fcs = [('x1', 'y1'), ('x2', 'y2')]
...
class MetaProcessingParams:
    len_to_make_trash_index = 15  # throw out tracks with length less or equal this parameter
